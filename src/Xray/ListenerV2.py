import os, json, base64, attributes, pyscreenrec, report, config, xray
...
class ListenerV2:
    ...
    def start_suite(self, name: str, attributes: attributes.StartSuiteAttributes):
    ...
    def end_suite(self, name: str, attributes: attributes.EndSuiteAttributes):
    ...
    def start_test(self, name: str, attributes: attributes.StartTestAttributes):
    ...
    def start_keyword(self, name: str, attributes: attributes.StartKeywordAttributes):
    ...
    def end_keyword(self, name: str, attributes: attributes.EndKeywordAttributes):
    ...
    def log_message(self, message: attributes.MessageAttributes):
    ...
    def message(self, message: attributes.MessageAttributes):
    ...
    def library_import(self, name: str, attributes: attributes.LibraryAttributes):
        """Called after a library has been imported."""

    def resource_import(self, name: str, attributes: attributes.ResourceAttributes):
        """Called after a resource file has been imported."""

    def variables_import(self, name: str, attributes: attributes.VariablesAttributes):
    ...
    def close(self):
        ...
        if config.Config.test_type() == "ROBOT":
            report.Report.robot(self.report)
            testExecutionId = xray.Xray.importExecutionRobot()
        ...
        for suite in report:
            for test in suite['tests']:
                if test['video'] and test['xraytest']:
                    xrayTestIssueId = xray.Xray.getTest(test['xraytest'])
                    id = xray.Xray.getTestRun(xrayTestIssueId, testExecutionId)
                    xray.Xray.addEvidenceToTestRun(id, 'Evidence_{}.mp4'.format(test['xraytest']), test['video'])
